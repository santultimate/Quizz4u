/// 📚 CULTURE GÉNÉRALE - PHASE 9 FINALE (3 questions pour 100%)
/// Traductions manuelles professionnelles
/// Focus: Questions finales pour compléter à 100%
/// Date: 17 octobre 2025

class CultureGeneralePhase9FinalTranslations {
  static const Map<String, Map<String, dynamic>> translations = {
    // Q1: Capitale du Japon
    "Quelle est la capitale du Japon ?": {
      'en': {
        'question': "What is the capital of Japan?",
        'answers': {
          'Tokyo': true,
          'Osaka': false,
          'Kyoto': false,
          'Hiroshima': false,
        },
      },
      'ar': {
        'question': "ما هي عاصمة اليابان؟",
        'answers': {
          'طوكيو': true,
          'أوساكا': false,
          'كيوتو': false,
          'هيروشيما': false,
        },
      },
      'zh': {
        'question': "日本的首都是哪里？",
        'answers': {
          '东京': true,
          '大阪': false,
          '京都': false,
          '广岛': false,
        },
      },
      'hi': {
        'question': "जापान की राजधानी क्या है?",
        'answers': {
          'टोक्यो': true,
          'ओसाका': false,
          'क्योटो': false,
          'हिरोशिमा': false,
        },
      },
      'es': {
        'question': "¿Cuál es la capital de Japón?",
        'answers': {
          'Tokio': true,
          'Osaka': false,
          'Kioto': false,
          'Hiroshima': false,
        },
      },
    },

    // Q2: Plus haute montagne
    "Quelle est la plus haute montagne du monde ?": {
      'en': {
        'question': "What is the highest mountain in the world?",
        'answers': {
          'Mount Everest': true,
          'K2': false,
          'Kangchenjunga': false,
          'Kilimanjaro': false,
        },
      },
      'ar': {
        'question': "ما هي أعلى جبل في العالم؟",
        'answers': {
          'جبل إيفرست': true,
          'كي 2': false,
          'كانشينجونغا': false,
          'كليمنجارو': false,
        },
      },
      'zh': {
        'question': "世界上最高的山是什么？",
        'answers': {
          '珠穆朗玛峰': true,
          'K2': false,
          '干城章嘉峰': false,
          '乞力马扎罗山': false,
        },
      },
      'hi': {
        'question': "दुनिया का सबसे ऊंचा पर्वत कौन सा है?",
        'answers': {
          'माउंट एवरेस्ट': true,
          'के2': false,
          'कंचनजंगा': false,
          'किलिमंजारो': false,
        },
      },
      'es': {
        'question': "¿Cuál es la montaña más alta del mundo?",
        'answers': {
          'Monte Everest': true,
          'K2': false,
          'Kangchenjunga': false,
          'Kilimanjaro': false,
        },
      },
    },

    // Q3: Qui a découvert l'Amérique
    "Qui a decouvert l'Amerique ?": {
      'en': {
        'question': "Who discovered America?",
        'answers': {
          'Christopher Columbus': true,
          'Vasco da Gama': false,
          'Marco Polo': false,
          'Ferdinand Magellan': false,
        },
      },
      'ar': {
        'question': "من اكتشف أمريكا؟",
        'answers': {
          'كريستوفر كولومبوس': true,
          'فاسكو دا غاما': false,
          'ماركو بولو': false,
          'فرديناند ماجلان': false,
        },
      },
      'zh': {
        'question': "谁发现了美洲？",
        'answers': {
          '克里斯托弗·哥伦布': true,
          '瓦斯科·达·伽马': false,
          '马可·波罗': false,
          '费迪南德·麦哲伦': false,
        },
      },
      'hi': {
        'question': "अमेरिका की खोज किसने की?",
        'answers': {
          'क्रिस्टोफर कोलंबस': true,
          'वास्को डी गामा': false,
          'मार्को पोलो': false,
          'फर्डिनेंड मैगलन': false,
        },
      },
      'es': {
        'question': "¿Quién descubrió América?",
        'answers': {
          'Cristóbal Colón': true,
          'Vasco da Gama': false,
          'Marco Polo': false,
          'Fernando de Magallanes': false,
        },
      },
    },
  };
}









